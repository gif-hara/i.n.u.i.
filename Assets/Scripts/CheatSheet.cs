...
namespace HK.Inui
{
    public sealed class CheatSheet : MonoBehaviour
    {
        ...
        void Start()
        {
			...
			builder.AppendLine(string.Format("\']\' {0} = 現在の状態から一番手前の\"{1}\"へ戻る", Inui.ReservedWord.WhileEnd, Inui.ReservedWord.WhileStart));
			builder.AppendLine(string.Format("\'.\' {0} = ポインタの指すメモリの値を出力する", Inui.ReservedWord.Echo));
            builder.AppendLine(string.Format("\'D\' {0} = 現在の状態をデバッグ表示する", Inui.ReservedWord.Print));
